...
def fetch_current_price(asset: str) -> float:
    """Get current price via CoinGecko simple/price."""
    ...
    if data and coin_id in data:
        ...
        if price:
            return float(price)
    # Fallback: try Hyperliquid meta
    return _hl_price_fallback(asset)
...
def fetch_hl_leaderboard_addresses() -> list:
    """
    Hyperliquid exposes a leaderboard endpoint via their public API.
    Returns list of wallet addresses of top traders.
    """
    ...
    if data and isinstance(data, dict):
        ...
        addrs = [r["ethAddress"] for r in rows[:TOP_N_TRADERS] if "ethAddress" in r]
...
def _hl_sample_addresses() -> list:
    """Known active Hyperliquid traders (public, on-chain)."""
    return [
...
def fetch_hl_all_positions(asset: str) -> list:
    ...
    clog(f"Hyperliquid: {len(all_pos)} positions for {asset}", Fore.GREEN)
    return all_pos
...
def fetch_dydx_all_positions(asset: str) -> list:
    ...
    if not addresses:
        clog("dYdX: no addresses found, skipping", Fore.YELLOW)
        return []
    all_pos = []
    ...
    clog(f"dYdX: {len(all_pos)} positions for {asset}", Fore.GREEN)
    return all_pos
